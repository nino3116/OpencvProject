...
def record_video(frame_generator, camera_name):
    """
    프레임 제너레이터와 카메라 이름을 받아 영상을 녹화하고 사람 인식 결과에 따라 스냅샷을 저장하는 함수.
    ...
    try:
        for frame, detections in frame_generator:
            ...
            frame_with_detections = frame.copy()  # 검출 결과를 그릴 프레임 복사
            ...
            if person_detected:
                ...
                cv.imwrite(str(snapshot_filename), frame_with_detections)
